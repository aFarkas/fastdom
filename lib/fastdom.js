...
FastDom.prototype.measure = FastDom.prototype.read = function(fn, ctx, args) {
  debug('measure');
  var task = { fn: fn, ctx: ctx, args: args };
...
FastDom.prototype.mutate = FastDom.prototype.write = function(fn, ctx, args) {
  debug('mutate');
  var task = { fn: fn, ctx: ctx, args: args };
...
function runTasks(tasks) {
  debug('run tasks');
  var task; while (task = tasks.shift()) task.fn.apply(task.ctx, task.args);
}
...
Sandbox.prototype.measure = Sandbox.prototype.read = function(fn, ctx, args) {
...
Sandbox.prototype.mutate = Sandbox.prototype.write = function(fn, ctx, args) {
...
[['write', 'mutate'], ['read', 'measure']].forEach(function(names) {
  var method = names[0] + (names[0] == 'read' ? 'er' : 'r');
  var alias = names[1] + 'r';
